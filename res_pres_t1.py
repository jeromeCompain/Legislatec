...
raw_file=open("Presidentielle_2017_Resultats_Tour_1_c.csv","r")
...
for line in raw_lines :
	champs=line.replace('É','E').replace(' ','_').split(";")
	for i in range(1,15):
		res_pres_t1[champs[0]][headers[i]]=champs[i]
	for i in [17,24,31,38,45,52,59,66,73,80,87]:
		res_pres_t1[champs[0]][champs[i].lower()]=champs[i+2]

clean_headers=["circo"]
for i in range(1,15):
	clean_headers.append(headers[i])
clean_headers.extend(["macron","le_pen","fillon","melenchon","hamon","dupont-aignan","lassalle","poutou","asselineau","arthaud","cheminade"])
clean_csv=open("res_pres2017_t1.csv","w")
clean_csv.write(";".join(clean_headers)+"\n")
for code in codes_circos :
	valeurs=[code]
	for champ in clean_headers[1:]:
		valeurs.append(res_pres_t1[code][champ])
